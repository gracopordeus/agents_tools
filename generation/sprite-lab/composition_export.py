...
def export_relationship(
    ...
    if not relationship_id or not character_id or not animation_id:
        raise ValueError("composição exige id, mesh principal e Action")
    ...
    animation_asset_id = str(animation.get("asset_id") or "")
    if not animation_asset_id:
        raise ValueError(f"animação sem asset pai: {animation_id}")

    character_path = _converted_model(character_id, assets)
    animation_path = _converted_model(animation_asset_id, assets)
